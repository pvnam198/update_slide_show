...
public class RangeSeekBar extends View {
    ...
    private int tickMarkTextSelectedColor;
    ...
    private ArrayList<String> processLabelValue;

    public void setProcessCustomValue(int value){
        int lastIndex = processLabelValue.size()-1;
        processLabelValue.remove(lastIndex);
        processLabelValue.add(String.valueOf(value));
        tickMarkTextArray = processLabelValue.toArray(new CharSequence[processLabelValue.size()]);
        invalidate();
    }

    public void setProcessDefaultLabelValue(){
        processLabelValue = new ArrayList<>();
        for (int i = 0; i < 6; i++){
            processLabelValue.add(String.valueOf(i+1));
        }
        tickMarkTextArray = processLabelValue.toArray(new CharSequence[processLabelValue.size()]);
    }
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            ...
            tickMarkTextSelectedColor = t.getColor(R.styleable.RangeSeekBar_rsb_tick_mark_text_selected_color, progressDefaultColor);
    ...
    protected void onDrawTickMark(Canvas canvas, Paint paint) {
        if (tickMarkTextArray != null) {
            ...
            int currentProgress = (int) getLeftSeekBar().getProgress();
            for (int i = 0; i < tickMarkTextArray.length; i++) {
                final String text2Draw = tickMarkTextArray[i].toString()+getContext().getString(R.string.s);
                ...
                if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
                    if (i == currentProgress) {
                        paint.setColor(tickMarkTextSelectedColor);
                    }

...
import com.example.slide.ui.edit_image.events.FlipHorizontallyEvent;
import com.example.slide.ui.edit_image.events.ZoomIconEvent;
...
public class StickerView extends RelativeLayout {
    ...
    private final List<BitmapFloatingItemIcon> icons = new ArrayList<>(4);
    ...
    private BitmapFloatingItemIcon currentIcon;
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void configDefaultIcons() {
        ...
        BitmapFloatingItemIcon deleteIcon = new BitmapFloatingItemIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_close_white_18dp),
                BitmapFloatingItemIcon.LEFT_TOP, BitmapFloatingItemIcon.REMOVE);
        deleteIcon.setIconEvent(new DeleteIconEvent());

        BitmapFloatingItemIcon zoomIcon = new BitmapFloatingItemIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_scale_white_18dp),
                BitmapFloatingItemIcon.RIGHT_BOTOM, BitmapFloatingItemIcon.ZOOM);
        zoomIcon.setIconEvent(new ZoomIconEvent());

        BitmapFloatingItemIcon flipIcon = new BitmapFloatingItemIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_flip_white_18dp),
                BitmapFloatingItemIcon.LEFT_BOTTOM, BitmapFloatingItemIcon.FLIP);
        flipIcon.setIconEvent(new FlipHorizontallyEvent());

        BitmapFloatingItemIcon editIcon = new BitmapFloatingItemIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.ic_edt),
                BitmapFloatingItemIcon.RIGHT_TOP, BitmapFloatingItemIcon.EDIT);
    ...
    public void setHandlingFloatingItem(FloatingItem handlingFloatingItem) {
    ...
    public void sendToLayer(int oldPos, int newPos) {
        if (floatingItems.size() >= oldPos && floatingItems.size() >= newPos) {
            FloatingItem s = floatingItems.get(oldPos);
            floatingItems.remove(oldPos);
            floatingItems.add(newPos, s);
            invalidate();
        }
    }
        ...
        if (handlingFloatingItem != null && !locked && (showBorder || showIcons)) {
            ...
            if (showIcons) {
                ...
                for (int i = 0; i < icons.size(); i++) {
                    BitmapFloatingItemIcon icon = icons.get(i);
                    switch (icon.getPosition()) {
                        ...
                        case BitmapFloatingItemIcon.RIGHT_TOP:
                            if ((handlingFloatingItem instanceof TextFloatingItem && icon.getTag().equals(BitmapFloatingItemIcon.EDIT)) || (handlingFloatingItem instanceof DrawableFloatingItem && icon.getTag().equals(BitmapFloatingItemIcon.FLIP))) {
                        ...
                        case BitmapFloatingItemIcon.LEFT_BOTTOM:
                            if (handlingFloatingItem instanceof BeautyFloatingItem) {
                                BeautyFloatingItem beautySticker = (BeautyFloatingItem) handlingFloatingItem;
                        ...
                        case BitmapFloatingItemIcon.RIGHT_BOTOM:
                            if ((handlingFloatingItem instanceof TextFloatingItem && icon.getTag().equals(BitmapFloatingItemIcon.ROTATE)) || (handlingFloatingItem instanceof DrawableFloatingItem && icon.getTag().equals(BitmapFloatingItemIcon.ZOOM))) {
                                configIconMatrix(icon, x4, y4, rotation);
                                icon.draw(canvas, borderPaint);
                            } else if (handlingFloatingItem instanceof BeautyFloatingItem) {
                                BeautyFloatingItem beautySticker = (BeautyFloatingItem) handlingFloatingItem;
                                if (beautySticker.getType() == BeautyFloatingItem.BUST_1) {
                                    configIconMatrix(icon, x4, y4, rotation);
                                    icon.draw(canvas, borderPaint);
                                } else if (beautySticker.getType() == BeautyFloatingItem.HIP_1 || beautySticker.getType() == BeautyFloatingItem.HIP_2 || beautySticker.getType() == BeautyFloatingItem.FACE) {
                                    configIconMatrix(icon, x4, y4, rotation);
                                    icon.draw(canvas, borderPaint);
                                }
                            }
    ...
    protected void configIconMatrix(@NonNull BitmapFloatingItemIcon icon, float x, float y,
                                    float rotation) {
        icon.setX(x);
        icon.setY(y);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_UP:
                if (currentMode == ActionMode.ZOOM_WITH_TWO_FINGER && handlingFloatingItem != null) {
                    if (onStickerOperationListener != null) {
    ...
    protected void onTouchUp(@NonNull MotionEvent event) {
        ...
        if (currentMode == ActionMode.ICON && currentIcon != null && handlingFloatingItem != null) {
        ...
        if (currentMode == ActionMode.DRAG && handlingFloatingItem != null) {
            if (onStickerOperationListener != null) {
    ...
    @Nullable
    protected BitmapFloatingItemIcon findCurrentIconTouched() {
        for (BitmapFloatingItemIcon icon : icons) {
    ...
    public void flip(@Nullable FloatingItem floatingItem, @Flip int direction) {
        if (floatingItem != null) {
    ...
    public boolean replace(@Nullable FloatingItem floatingItem) {
        return replace(floatingItem, true);
    }
    ...
    public boolean replace(@Nullable FloatingItem floatingItem, boolean needStayState) {

        if (handlingFloatingItem == null)
            handlingFloatingItem = lastHandlingFloatingItem;

        if (handlingFloatingItem != null && floatingItem != null) {
            float width = getWidth();
            float height = getHeight();
            if (needStayState) {
                floatingItem.setMatrix(handlingFloatingItem.getMatrix());
                floatingItem.setFlippedVertically(handlingFloatingItem.isFlippedVertically());
                floatingItem.setFlippedHorizontally(handlingFloatingItem.isFlippedHorizontally());
            } else {
                ...
                floatingItem.getMatrix().postTranslate(offsetX, offsetY);

                float scaleFactor;
                if (width < height) {
                    if (handlingFloatingItem instanceof TextFloatingItem)
                ...
                floatingItem.getMatrix().postScale(scaleFactor / 2f, scaleFactor / 2f, width / 2f, height / 2f);
            }
            int index = floatingItems.indexOf(handlingFloatingItem);
            floatingItems.set(index, floatingItem);
    ...
    public void removeBitmapStickers(){
        for (int i = 0; i < floatingItems.size(); i++){
            FloatingItem floatingItem = floatingItems.get(i);
            if (floatingItem instanceof DrawableFloatingItem){
                floatingItems.remove(floatingItem);
                i--;
            }
        }
    ...
    @NonNull
    public StickerView addSticker(@NonNull FloatingItem floatingItem) {
        return addSticker(floatingItem, FloatingItem.Position.CENTER);
    }

    public StickerView addSticker(@NonNull final FloatingItem floatingItem,
    ...
    @NonNull
    public List<BitmapFloatingItemIcon> getIcons() {
        return icons;
    }

    public void setIcons(@NonNull List<BitmapFloatingItemIcon> icons) {
        this.icons.clear();
